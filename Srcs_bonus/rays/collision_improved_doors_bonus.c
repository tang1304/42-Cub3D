...
static int	set_miss_return_e_w(t_data *data, t_ray *ray, t_coord_f *miss, int check)
{
	if (ray->side_hit == 1)
	{
		if (data->arr[(int)ray->cell.y][(int)ray->cell.x] == '1' || \
...
static int	set_miss_return_n_s(t_data *data, t_ray *ray, t_coord_f *miss, int check)
{
	if (ray->side_hit == 3)
	{
		if (data->arr[(int)ray->cell.y][(int)ray->cell.x] == '1' || \
